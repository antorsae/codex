...
These actions also return `selectedAccount`, containing the selected alias and authenticated user/workspace identity without credentials. Clients can use that alias for a scoped `usage` read and follow `thread/accountPool/updated` selection notifications to keep account displays current.
